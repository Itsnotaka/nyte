...
describe("GET /api/sync/poll", () => {
  ...
  it("returns 502 and writes delegation audit when sync runtime outage persists", async () => {
    process.env.NYTE_RUNTIME_DELEGATE_SYNC = "true";
    process.env.NYTE_RUNTIME_URL = "https://runtime.nyte.dev";
    let callCount = 0;
    globalThis.fetch = async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/sync/poll", {
        headers: {
          "x-forwarded-for": "192.0.2.101",
          "x-request-id": "req_ingest_outage_1",
        },
      }),
    );
    const body = (await response.json()) as { error: string };
    ...
    const runtimeAuditRows = await db
      .select()
      .from(auditLogs)
      .where(eq(auditLogs.action, "runtime.delegate.ingest.dispatch_error"));

    expect(response.status).toBe(502);
    expect(body.error).toBe("Runtime temporarily unavailable.");
    expect(callCount).toBe(2);
    expect(runtimeAuditRows.length).toBe(1);
    expect(runtimeAuditRows[0]?.targetId).toBe("req_ingest_outage_1");
