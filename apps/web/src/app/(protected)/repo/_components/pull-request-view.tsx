      ...
      <div className="mx-auto flex h-full w-full max-w-[1280px] flex-col gap-4 px-4 pb-6 pt-4 sm:px-6">
        <header className="space-y-2">
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            {pullRequest.head.ref} to {pullRequest.base.ref} · updated {formatUpdated(pullRequest.updated_at)}
          </p>
        </header>
...
function DiffFile({
  ...
  const annotations = React.useMemo<DiffLineAnnotation<AnnotationPayload>[]>(() => {
    const existing: DiffLineAnnotation<AnnotationPayload>[] = reviewComments.flatMap(
      (comment) => {
