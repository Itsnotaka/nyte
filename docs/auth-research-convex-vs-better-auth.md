...
The Nyte app currently uses **Better Auth** with Postgres/Drizzle, Google OAuth
(including Gmail/Calendar scopes), and Next.js. This document compares Convex
Auth with Better Auth and outlines migration paths.
...
2. Callback URL is the Convex HTTP Actions URL:
   `https://<deployment>.convex.site/api/auth/callback/google`
3. Credentials stored via Convex env vars: `AUTH_GOOGLE_ID`,
   `AUTH_GOOGLE_SECRET`
...
- **Behavior**: JWT is the source of truth; deleting a session does not sign the
  user out until the JWT expires
- **Multiple sessions**: Supported per user
- **Custom domain**: `CUSTOM_AUTH_SITE_URL` for production OAuth consent screen

### Accessing Auth State

- **Backend**: `getAuthUserId()`, `getAuthSessionId()`,
  `ctx.auth.getUserIdentity()`
- **Frontend**: `Authenticated`, `Unauthenticated`, `AuthLoading` wrappers;
...
- **Package**: `better-auth`
- **Backend**: Runs in your app (Next.js route handlers) or as a standalone
  server
...
- **Client**: `authClient.signIn.social()`, `authClient.signOut()`,
...
Nyte uses extended Google scopes for Gmail and Calendar. Convex Auth uses
standard Auth.js providers; custom scopes require provider configuration (e.g.
`scope` in the Google provider). Better Auth supports this directly in
`socialProviders.google.scope`.

---

## 4. Convex + Better Auth Integration

Convex provides an official integration: **Better Auth on Convex**
...
- Sign-in/sign-out must be done from the client (no HTTP cookies for Convex
  RPCs)
- Route handlers are mounted on Convex HTTP router and proxied from Next.js

**Use when**: You want Convex as the backend but keep Better Auth’s features and
API.
...
- **Notes**: Current setup is production-ready and supports Gmail/Calendar
  scopes
...
4. **Replace Better Auth usage**
   - Session: `auth.api.getSession()` → `ctx.auth.getUserIdentity()` in Convex
     functions
   - Access token: use Auth.js provider tokens or custom storage; Convex Auth
     does not expose `getAccessToken` directly—you may need to store tokens in
     Convex tables via `createOrUpdateUser`/`afterUserCreatedOrUpdated`
     callbacks
   - Client: `authClient.signIn.social()` → `signIn("google")` from
     `useAuthActions()`
   - tRPC context: obtain `userId` from Convex identity instead of Better Auth
     session

5. **Data migration**
   - Map Better Auth `users`/`accounts`/`sessions` to Convex
...
**Risks**: Beta status; access token handling for Gmail/Calendar may need custom
logic.
...
1. **Adopt Convex**
   - Add Convex project
   - Install `@convex-dev/better-auth` and `better-auth` (pinned version per
     docs)
...
5. **Update server usage**
   - `auth.api.*` calls move into Convex functions using
     `authComponent.getAuth(createAuth, ctx)`
   - tRPC or other server code must obtain session/token from Convex or pass
     through Convex
...
**Notes**: Keeps Better Auth API and Gmail/Calendar scopes; adds Convex as
backend. Server-side sign-in/sign-out is not supported—must use client
`authClient.signIn.*`.
...
For Nyte’s current Gmail/Calendar integration and `auth.api.getAccessToken()`
usage, **Path C (Convex + Better Auth)** is the most straightforward way to
adopt Convex while preserving existing auth behavior.
